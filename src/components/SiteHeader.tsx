      ...
      {mobileMenuOpen && (
        <div id="mobile-menu" role="navigation" aria-label="Mobile menu" className={`fixed inset-x-0 top-16 z-40 border-b md:hidden ${isScrolled ? 'border-charcoal/10 bg-white shadow-lg' : 'border-white/10 bg-[#0F0F10]'}`}>
          <div className="mx-auto max-w-6xl flex flex-col px-6 py-4 gap-1">
            <Link href="/#what-you-get" onClick={() => setMobileMenuOpen(false)} className={`text-[15px] font-medium py-2.5 px-3 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-cran/50 focus:ring-inset ${isScrolled ? 'text-charcoal hover:bg-charcoal/5' : 'text-white hover:bg-white/[0.04]'}`}>Features</Link>
